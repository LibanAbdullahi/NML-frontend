...
const DoctorDashboard: React.FC<DoctorDashboardProps> = ({ token }) => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            setIsLoading(true);
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/users/patients`, {
    ...
    const handleSaveRecord = async () => {
        ...
        try {
            ...
            await axios.post(url, { ...recordData, userId: selectedUserId }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setRecordData({});
            setError(null);
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.formGroup}>
                ...
                <select id="userSelect" onChange={handleUserSelect} value={selectedUserId}>
            ...
            <div className={styles.recordForm}>
                ...
                <div className={styles.formGroup}>
                    ...
                    <textarea 
                        name="labResults"
                        value={typeof recordData.labResults === 'string' ? recordData.labResults : ''}
                        onChange={handleInputChange}
                    />
                </div>
                {/* Add more fields for currentMedications, allergies, labResults */}
                <button onClick={handleSaveRecord} disabled={!selectedUserId}>
                    Create Record
                </button>
            </div>
            ...
            {selectedUserId && <UserDashboard userId={selectedUserId} token={token} />}
        </div>
    );
};
